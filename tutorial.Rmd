...
Now that you've selected your file names, you can begin running the main functions. For a detailed description of what they do and their parameters, visit the [functions page](https://github.com/nclark-lab/erc/blob/main/functions.md). Here, your tree file is read in with `readTrees`, and the trees are transformed (via a square root transform) with `transformPaths`. 
...
Next, with `getAllResiduals`, Relative Evolutionary Rates (RERs) are calculated, and then those rates are formatted into a matrix with `getRMat`
```{r message = FALSE, warning = FALSE, cache = TRUE}
...
```{r}
corres=computeERC(rMat, comptrees, clusterListOutput = clusterList,
